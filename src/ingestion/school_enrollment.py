"""Ingest school enrollment data from Census ACS into Bronze.

Uses the Census ACS 5-Year API to retrieve county-level school enrollment
(variable B14001_002E = total enrolled in school, ages 3+).

URL pattern:
  https://api.census.gov/data/{year}/acs/acs5?get=B14001_002E,NAME&for=county:*&in=state:*
"""

import logging
from datetime import datetime, timezone, date
from typing import Any

import requests

from src.common.fips import normalize_fips

logger = logging.getLogger(__name__)


def _fetch_enrollment(year: int) -> list[dict[str, Any]]:
    """Fetch county-level school enrollment from Census ACS API."""
    url = (
        f"https://api.census.gov/data/{year}/acs/acs5"
        f"?get=B14001_002E,NAME&for=county:*&in=state:*"
    )
    response = requests.get(url, timeout=120)
    response.raise_for_status()
    data = response.json()

    # First row is the header
    header = data[0]
    rows: list[dict[str, Any]] = []
    for record in data[1:]:
        row = dict(zip(header, record))
        state_fips = row["state"]
        county_fips = row["county"]
        fips = normalize_fips(state_fips + county_fips)

        enrollment_raw = row.get("B14001_002E")
        if enrollment_raw is None or enrollment_raw in ("", "-"):
            continue
        total_enrollment = int(enrollment_raw)

        rows.append({
            "fips": fips,
            "report_year": year,
            "total_enrollment": total_enrollment,
            "data_source": "census_acs",
        })
    return rows


def ingest(spark, year: int, catalog: str | None = None):
    """Ingest school enrollment data into Bronze."""
    from pyspark.sql.functions import lit, current_timestamp
    if catalog is None:
        from src.common.config import CATALOG
        catalog = CATALOG
    from src.common.ingestion_logger import log_ingestion

    started_at = datetime.now(timezone.utc)
    try:
        rows = _fetch_enrollment(year)
        if not rows:
            log_ingestion(spark, "school_enrollment", "success", 0,
                         started_at=started_at, catalog=catalog)
            return

        df = spark.createDataFrame(rows)
        df = (
            df.withColumn("source_date", lit(date(year, 1, 1)))
              .withColumn("ingested_at", current_timestamp())
        )
        df.write.mode("append").saveAsTable(f"{catalog}.bronze.school_enrollment")

        log_ingestion(spark, "school_enrollment", "success", len(rows),
                     started_at=started_at, catalog=catalog)
    except Exception as e:
        log_ingestion(spark, "school_enrollment", "failure",
                     error_msg=str(e)[:500], started_at=started_at, catalog=catalog)
        raise
