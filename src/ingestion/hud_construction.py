"""Ingest HUD construction activity data into Bronze."""
...
def build_download_url(year: int, quarter: int) -> str:
    """Build the HUD construction data download URL."""
    return f"https://www.huduser.gov/portal/datasets/socds/construction_{year}q{quarter}.csv"


def parse_hud_construction_csv(filepath: str) -> list[dict[str, Any]]:
    """Parse HUD construction CSV into list of dicts."""
    rows = []
    with open(filepath, "r") as f:
        ...
        for record in reader:
            ...
            rows.append({
                ...
                "permitted_units": int(record["permitted_units"].strip()),
                "structure_type": record["structure_type"].strip(),
            })
    return rows
...
def download_and_parse(year: int, quarter: int) -> list[dict[str, Any]]:
    ...
    try:
        return parse_hud_construction_csv(tmp_path)
    finally:
        os.unlink(tmp_path)
